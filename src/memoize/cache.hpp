...
template <timeout_policy_t tp, class fptr_t, class... Keys> struct Cache
{
    ...
    static_assert(use_unordered || use_ordered,
                  "Either all passed types must be (Hashable and Compareable) or all must be LessThanCompareable.");
    static_assert((tp == timeout_policy_t::NEVER) ^ (tp == timeout_policy_t::LRU),
                  "Invalid enum value for timeout_policy_t tp.");

    typedef std::tuple<Keys...> key_t;
    typedef std::tuple<std::decay_t<Keys>...> decayed_key_t;

    typedef std::conditional_t<use_unordered, std::unordered_map<key_t, R>, std::map<key_t, R>> cache_map_t;

    typedef std::list<R>
        internal_cache_t; // use list for constant insert operations, traversion done in const time with iterators
    typedef typename internal_cache_t::iterator internal_cache_it_t;

    typedef std::conditional_t<use_unordered, std::unordered_map<key_t, internal_cache_it_t>,
                               std::map<key_t, internal_cache_it_t>>
        cache_vector_t;

    typedef std::conditional23_t<tp == timeout_policy_t::NEVER, tp == timeout_policy_t::LRU, cache_map_t,
                                 cache_vector_t,
                                 void>
        cache_t; // void means error, already caught through static_assert
    ...
    // const& -> const&
    template <class T> inline constexpr T const &getKey(T const &key)
    {
        return key;
    }
